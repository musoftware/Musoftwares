...
class AutoSmsController extends Controller
{
    ...
    public function index()
    {
        ...
        $devices = AutoSmsDevice::where('user_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->get();

        $webhook = AutoSmsWebhook::where('user_id', $user->id)
            ->where('is_active', true)
            ->first();

        $recentTransactions = AutoSmsTransaction::where('user_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->limit(5)
            ->get();
        ...
        $stats = [
            ...
            'total_transactions' => AutoSmsTransaction::where('user_id', $user->id)->count(),
    ...
    public function indexClassic()
    {
        ...
        return view('client.auto-sms.index-old', compact('devices', 'webhook', 'token'));
    }
    ...
    public function generateQrCode(Request $request)
    {
        ...
        $device = AutoSmsDevice::updateOrCreate(
            [
                'user_id' => $user->id,
                'status' => 'pending'
            ],
        ...
        $qrData = json_encode([
            ...
            'connection_code' => $connectionCode,
            'user_id' => $user->id,
            'api_url' => url('/api/auto-sms/connect'),
    ...
    {
        $device = AutoSmsDevice::where('user_id', Auth::id())
            ->with([
        ...
        $transactions = AutoSmsTransaction::where('device_id', $device->id)
    ...
    public function updateWebhook(Request $request)
    {
        ...
        $existingWebhook = AutoSmsWebhook::where('user_id', $user->id)
            ->where('is_active', true)
            ->first();
    ...
    {
        $webhook = AutoSmsWebhook::where('id', $id)
     ...
     * GET /client/auto-sms/verification-secret
     */
    public function getVerificationSecret(Request $request)
    {
        $user = Auth::user();
        $secret = $user->getAutoSmsVerificationSecret();
     ...
     * POST /client/auto-sms/verification-secret/regenerate
     */
    public function regenerateVerificationSecret(Request $request)
    {
        $user = Auth::user();
        $newSecret = $user->regenerateAutoSmsVerificationSecret();
     ...
     * GET /client/auto-sms/verification
     */
     ...
     * GET /client/auto-sms/documentation
     */
     ...
     * GET /client/auto-sms/integration-tester
     */
     ...
     * POST /client/auto-sms/integration-tester/run
     */
     ...
     * GET /client/auto-sms/verification-secret-page
     */
     ...
     * GET /client/auto-sms/wallets
     */
    public function wallets()
    {
        $user = Auth::user();

        $wallets = AutoSmsWallet::where('user_id', $user->id)
            ->orderBy('created_at', 'desc')
            ->get();
    ...
    /**
     * Delete a wallet
     * DELETE /client/auto-sms/wallets/{id}
     */
    public function deleteWallet($id)
    {
        $wallet = AutoSmsWallet::where('user_id', Auth::id())
            ->findOrFail($id);
    ...
    public function testMode()
    {
        ...
        $testTransactionsCount = AutoSmsTransaction::where('user_id', $user->id)
            ->where('is_test', true)
            ->count();
        ...
        $testTransactions = AutoSmsTransaction::where('user_id', $user->id)
            ->where('is_test', true)
            ->orderBy('created_at', 'desc')
            ->limit(10)
            ->get();
    ...
    /**
     * Toggle test mode
     * POST /client/auto-sms/test-mode/toggle
     */
     ...
     * POST /client/auto-sms/test-mode/create-transaction
     */
    /**
     * Create test transaction
     * POST /client/auto-sms/test-mode/create-transaction
    ...
    public function createTestTransaction(Request $request)
    {
        ...
        $device = AutoSmsDevice::where('user_id', $user->id)->first();
        if (!$device) {
            $device = AutoSmsDevice::create([
                'user_id' => $user->id,
    ...
    /**
     * Send test webhook
     * POST /client/auto-sms/test-mode/send-webhook
     */
    public function sendTestWebhook(Request $request)
    {
        ...
        $device = AutoSmsDevice::where('user_id', $user->id)->first();
        $transaction = AutoSmsTransaction::create([
            'user_id' => $user->id,
    ...
    /**
     * Clear test data
     * DELETE /client/auto-sms/test-mode/clear-data
     */
    public function clearTestData()
    {
        ...
        AutoSmsTransaction::where('user_id', $user->id)
            ->where('is_test', true)
            ->delete();
