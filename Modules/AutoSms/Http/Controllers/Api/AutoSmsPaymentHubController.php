...
namespace Modules\AutoSms\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use Modules\AutoSms\Models\AutoSmsDevice;
use Modules\AutoSms\Models\AutoSmsTransaction;
use Modules\AutoSms\Models\AutoSmsWallet;
use Modules\AutoSms\Models\AutoSmsOrderLink;
use App\Models\User;
...
use Carbon\Carbon;
use Modules\AutoSms\Services\AutoSmsParserService;

class AutoSmsPaymentHubController extends Controller
{
    /**
     * Connect device using connection code
     * POST /api/auto-sms/connect
    ...
    public function connect(Request $request)
    {
        ...
        $device = AutoSmsDevice::where('connection_code', $request->connection_code)
        ...
        $existingDevice = AutoSmsDevice::where('device_token', $request->device_token)
            ->where('id', '!=', $device->id)
            ->first();
     ...
     * GET /api/auto-sms/allowed-senders
    ...
    public function getAllowedSenders(Request $request)
    {
        ...
        $allowedSenders = config('auto-sms.allowed_senders', [
    ...
    public function receiveSms(Request $request)
    {
        ...
        $device = AutoSmsDevice::where('device_token', $request->device_token)
        ...
        if ($request->has('message_id') && !empty($request->message_id)) {
            $existingTransaction = AutoSmsTransaction::where('message_id', $request->message_id)
        ...
        if (!$request->is_test) {
            $existingMessageTransaction = AutoSmsTransaction::where('sms_message', $request->message)
                ->where('device_id', $device->id)
        ...
        $parserService = app(AutoSmsParserService::class);
...
null";
    ...
    protected function normalizeAITransactionData(array $data, string $sender): ?array
    {
        ...
        // Validate amount
        $minAmount = config('auto-sms.min_transaction_amount', 1);
        $maxAmount = config('auto-sms.max_transaction_amount', 1000000);
     ...
     */
    protected function getTransactionDetectionDebugInfo(string $message, string $sender, ?AutoSmsDevice $device = null): array
    {
        $debugInfo = [
            'reasons' => [],
            'extracted_data' => [],
        ...
        $exclusionPatterns = config('auto-sms.exclusion_patterns', []);
        $isPromotional = false;
        $matchedPattern = null;
        ...
        if ($device && $device->user) {
            $geminiAvailable = !empty($device->user->gemini_api) && config('auto-sms.enable_gemini_extraction', true);
        }
    ...
    protected function checkForSpoofing(string $sender, ?float $currentBalance, float $currentAmount, int $userId, ?int $smsTimestamp = null): array
    {
        ...
        $tolerance = config('auto-sms.spoofing_tolerance', 100.00);
        $reasons = [];
        ...
        if ($smsTimestamp) {
            ...
            $previousTransaction = AutoSmsTransaction::where('sender', $sender)
        ...
        if ($smsTimestamp) {
            ...
            // Filter by sender (wallet) not phone_number
            $nextTransaction = AutoSmsTransaction::where('sender', $sender)
                ->where('user_id', $userId)
                ->where('balance', '!=', null)
                ->where('is_spoofed', false)
                ->where(function ($query) use ($smsTimestamp, $timestampDate) {
     ...
     */
    protected function processTransaction(AutoSmsDevice $device, array $transactionData, array $smsData): void
    {
        try {
            $user = $device->user;
            ...
            if ($phoneNumber) {
                ...
                $orderLink = AutoSmsOrderLink::where('user_id', $user->id)
            ...
            try {
                $columns = Schema::getColumnListing('auto_sms_transactions');
            ...
            $autoSmsTransaction = AutoSmsTransaction::create($transactionCreateData);

            Log::info('AutoSMS Payment Hub - Transaction Created', [
                'user_id' => $user->id,
                'device_id' => $device->id,
                'auto_sms_transaction_id' => $autoSmsTransaction->id,
                'message_id' => $smsData['message_id'] ?? null,
                'sms_timestamp' => isset($smsData['timestamp']) ? intval($smsData['timestamp']) : null,
                'amount' => $transactionData['amount'],
            ...
            \App\Http\Controllers\Api\AutoSmsWebhookController::sendTransactionWebhook($autoSmsTransaction);
     ...
     * POST /api/auto-sms/verify-transaction
    ...
    public function verifyTransaction(Request $request)
    {
        ...
        $transaction = AutoSmsTransaction::with('device') // Eager load device for security check
    ...
    public function linkOrder(Request $request)
    {
        ...
        $orderLink = AutoSmsOrderLink::create([
            'user_id' => $user->id,
     ...
     * POST /api/auto-sms/public/link-order
    ...
    public function linkOrderPublic(Request $request)
    {
        ...
        // Create AutoSmsOrderLink record
        $orderLink = AutoSmsOrderLink::create([
            'user_id' => $user->id,
            'order_id' => (string) $request->order_id,
            'phone_number' => $phoneNumber,
     ...
     * GET /api/auto-sms/debug/empty-phone-numbers
     * POST /api/auto-sms/debug/empty-phone-numbers?update=true (to update transactions)
    ...
    public function debugEmptyPhoneNumbers(Request $request)
    {
        ...
        $transactions = AutoSmsTransaction::query()
     ...
     * GET /api/auto-sms/get-random-wallet
     * Public endpoint (no auth required)
     */
    public function getRandomWallet(Request $request)
    {
        // Get all active wallets
        $wallets = AutoSmsWallet::where('is_active', true)
            ->inRandomOrder()
            ->first();
     ...
     * POST /api/auto-sms/verify-payment
     * Public endpoint (no auth required)
     */
    public function verifyPayment(Request $request)
    {
        $request->validate([
            'phone_number' => 'required|string|max:20',
            'wallet_id' => 'nullable|integer|exists:auto_sms_wallets,id',
        ]);
        ...
        if ($walletId) {
            $wallet = AutoSmsWallet::where('id', $walletId)
        ...
        $transaction = AutoSmsTransaction::where('phone_number', $phoneNumber)
