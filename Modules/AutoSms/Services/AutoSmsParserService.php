...
namespace Modules\AutoSms\Services;
...
class AutoSmsParserService
{
    /**
     * Detect transaction from SMS message
     */
    public function detectTransaction(string $message, string $sender, ?AutoSmsDevice $device = null): ?array
    {
        // Try AI extraction first if enabled and user has API key
        if (config('auto-sms.enable_gemini_extraction', true) && $device && $device->user) {
...
null";
    ...
    protected function extractTransactionWithRegex(string $message, string $sender): ?array
    {
        ...
        // Bank transfers (with reference number) don't require balance
        if ($balance === null && $phoneNumber && !$referenceNumber) {
            Log::info('AutoSMS Payment Hub - Wallet transaction rejected: No balance found in SMS', [
                'sender' => $sender,
                'phone_number' => $phoneNumber,
                'amount' => $amount,
    ...
    {
        return preg_match('/(?:برقم\s+مرجعي|reference|ref)[\s:]+([a-zA-Z0-9]+)/i', $message) === 1;
    }
     ...
     */
    public function getDebugInfo(string $message, string $sender, ?AutoSmsDevice $device = null): array
    {
        $debugInfo = [
