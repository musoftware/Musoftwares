...
<body>
    <div class="container">
        <div class="header">
            <h1>{{ __('erp.you_are_invited') }}</h1>
        </div>
        <div class="content">
            <p>{{ __('erp.hello') }} <strong>{{ $member->name }}</strong>,</p>
            <p>{{ __('erp.invite_email_body', ['tenant' => $member->tenant->name]) }}</p>
            <p>{{ __('erp.invite_email_instruction') }}</p>
            
            <div class="button-wrapper">
                <a href="{{ $inviteUrl }}" class="button">{{ __('erp.accept_invitation') }}</a>
            </div>

            <p>{{ __('erp.invite_email_expires_warning') }}</p>
        </div>
        <div class="footer">
            <p>&copy; {{ date('Y') }} {{ config('app.name') }}. {{ __('erp.all_rights_reserved') }}</p>
        </div>
    </div>
</body>
</html>
