...
class Project extends Model
{
    ...
    public function hour_rate_client()
    {
        if ($this->client->currency == 2) {
            ...
            $item = \Modules\Core\Models\GoldWorldPrice::query()
            ...
            $price_21 = \Modules\Core\Models\GoldPrice::query()->where(DB::raw('DATE(price_date)'), $item->price_date)->select(DB::raw('avg(price_21k) as price_21k'))->groupBy(DB::raw('DATE(price_date)'))->first();
...
//        if ($finance->count() == 0) {
//            $finance_row = new \Modules\Core\Models\ProjectFinanceSummary();
//        } else {
...
//    {
//        foreach (\Modules\Core\Models\Currency::as_array() as $item) {
