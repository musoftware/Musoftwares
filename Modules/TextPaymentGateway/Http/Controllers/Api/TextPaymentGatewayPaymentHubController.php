...
use Modules\TextPaymentGateway\Models\TextPaymentGatewayOrderLink;
use App\Models\User;
...
use Carbon\Carbon;
use Modules\TextPaymentGateway\Services\TextPaymentGatewayParserService;

class TextPaymentGatewayPaymentHubController extends Controller
{
    /**
     * Connect device using connection code
     * POST /api/text-payment-gateway/connect
    ...
    public function connect(Request $request)
    {
        ...
        $device = TextPaymentGatewayDevice::where('connection_code', $request->connection_code)
        ...
        $existingDevice = TextPaymentGatewayDevice::where('device_token', $request->device_token)
            ->where('id', '!=', $device->id)
            ->first();
     ...
     * GET /api/text-payment-gateway/allowed-senders
    ...
    public function getAllowedSenders(Request $request)
    {
        ...
        $allowedSenders = config('text-payment-gateway.allowed_senders', [
     ...
     * POST /api/text-payment-gateway/sms
    ...
    public function receiveSms(Request $request)
    {
        ...
        $device = TextPaymentGatewayDevice::where('device_token', $request->device_token)
        ...
        if ($request->has('message_id') && !empty($request->message_id)) {
            $existingTransaction = TextPaymentGatewayTransaction::where('message_id', $request->message_id)
        ...
        if (!$request->is_test) {
            $existingMessageTransaction = TextPaymentGatewayTransaction::where('sms_message', $request->message)
                ->where('device_id', $device->id)
        ...
        $parserService = app(TextPaymentGatewayParserService::class);
...
null";
     ...
     */
    protected function getTransactionDetectionDebugInfo(string $message, string $sender, ?TextPaymentGatewayDevice $device = null): array
    {
        $debugInfo = [
            'reasons' => [],
            'extracted_data' => [],
    ...
    protected function checkForSpoofing(string $sender, ?float $currentBalance, float $currentAmount, int $userId, ?int $smsTimestamp = null): array
    {
        ...
        if ($smsTimestamp) {
            ...
            $previousTransaction = TextPaymentGatewayTransaction::where('sender', $sender)
        ...
        if ($smsTimestamp) {
            ...
            // Filter by sender (wallet) not phone_number
            $nextTransaction = TextPaymentGatewayTransaction::where('sender', $sender)
                ->where('user_id', $userId)
                ->where('balance', '!=', null)
                ->where('is_spoofed', false)
                ->where(function ($query) use ($smsTimestamp, $timestampDate) {
     ...
     */
    protected function processTransaction(TextPaymentGatewayDevice $device, array $transactionData, array $smsData): void
    {
        try {
            $user = $device->user;
            ...
            if ($phoneNumber) {
                ...
                $orderLink = TextPaymentGatewayOrderLink::where('user_id', $user->id)
            ...
            $autoSmsTransaction = TextPaymentGatewayTransaction::create($transactionCreateData);
            ...
            \App\Http\Controllers\Api\TextPaymentGatewayWebhookController::sendTransactionWebhook($autoSmsTransaction);
     ...
     * POST /api/text-payment-gateway/verify-transaction
    ...
    public function verifyTransaction(Request $request)
    {
        ...
        $transaction = TextPaymentGatewayTransaction::with('device') // Eager load device for security check
     ...
     * POST /api/text-payment-gateway/link-order
    ...
    public function linkOrder(Request $request)
    {
        ...
        $orderLink = TextPaymentGatewayOrderLink::create([
            'user_id' => $user->id,
     ...
     * POST /api/text-payment-gateway/public/link-order
    ...
    public function linkOrderPublic(Request $request)
    {
        ...
        // Create TextPaymentGatewayOrderLink record
        $orderLink = TextPaymentGatewayOrderLink::create([
            'user_id' => $user->id,
            'order_id' => (string) $request->order_id,
            'phone_number' => $phoneNumber,
     ...
     * GET /api/text-payment-gateway/debug/empty-phone-numbers
     * POST /api/text-payment-gateway/debug/empty-phone-numbers?update=true (to update transactions)
    ...
    public function debugEmptyPhoneNumbers(Request $request)
    {
        ...
        $transactions = TextPaymentGatewayTransaction::query()
     ...
     * GET /api/text-payment-gateway/get-random-wallet
     * Public endpoint (no auth required)
     */
    public function getRandomWallet(Request $request)
    {
        // Get all active wallets
        $wallets = TextPaymentGatewayWallet::where('is_active', true)
            ->inRandomOrder()
            ->first();
     ...
     * POST /api/text-payment-gateway/verify-payment
     * Public endpoint (no auth required)
     */
    public function verifyPayment(Request $request)
    {
        ...
        if ($walletId) {
            $wallet = TextPaymentGatewayWallet::where('id', $walletId)
        ...
        $transaction = TextPaymentGatewayTransaction::where('phone_number', $phoneNumber)
