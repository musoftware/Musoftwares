...
class SmsPaymentGatewayParserService
{
    /**
     * Detect transaction from SMS message
     */
    public function detectTransaction(string $message, string $sender, ?SmsPaymentGatewayDevice $device = null): ?array
    {
        // Try AI extraction first if enabled and user has API key
        if (config('sms-payment-gateway.enable_gemini_extraction', true) && $device && $device->user) {
...
null";
     ...
     */
    public function getDebugInfo(string $message, string $sender, ?SmsPaymentGatewayDevice $device = null): array
    {
        $debugInfo = [
