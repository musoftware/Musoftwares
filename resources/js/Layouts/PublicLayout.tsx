...
export default function PublicLayout({ children, auth: propAuth }: PublicLayoutProps) {
    ...
    return (
        <div className="flex min-h-screen flex-col bg-[#ffffff] text-[#1d1d1f] antialiased font-sans selection:bg-[#0071e3]/20 selection:text-[#0071e3]">

            {/* Top Navigation Header (Apple Glassmorphism Light) */}
            <header
                className={`sticky top-0 w-full transition-all duration-200 border-b border-black/5 bg-white/80 backdrop-blur-md z-50`}
            ...
            >
                <div className="mx-auto flex max-w-[1280px] items-center justify-between px-6 sm:px-10 min-h-[56px] py-3">
                    ...
                    <div className="flex items-center gap-8">
                        <SafeLink href="/" className="flex items-center space-x-2.5 rtl:space-x-reverse group focus:outline-none shrink-0" title="Musoftwares Studio">
                            <svg className="w-5 h-5 fill-[#1d1d1f] group-hover:text-[#000000] transition-colors" viewBox="0 0 307 307" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className="text-[19px] sm:text-[21px] font-semibold tracking-[-0.03em] text-[#1d1d1f]">
                                Musoftware
                            </span>
                            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[#f5f5f7] border border-black/5 text-[#1d1d1f]/70">
                                STUDIO
                            </span>
                        </SafeLink>

                        {/* Desktop Navigation */}
                        <nav className="hidden lg:flex items-center space-x-6 rtl:space-x-reverse text-[13px] font-normal text-[#1d1d1f]/75">
                            {navItems.map((item) => (
                                <div key={item.id} className="relative">
                                    ...
                                    {item.items.length > 0 && activeDropdown === item.id && (
                                        <div
                                            ...
                                            onMouseLeave={() => setActiveDropdown(null)}
                                            className="absolute left-0 rtl:left-auto rtl:right-0 top-full mt-2 w-80 bg-white/95 backdrop-blur-xl border border-black/10 rounded-2xl p-2 shadow-[0_20px_50px_rgba(0,0,0,0.12)] animate-in fade-in slide-in-from-top-2 duration-150 z-50"
                                        >
                                            {item.items.map((subItem) => (
                                                <SafeLink
                                                    key={subItem.href}
                                                    href={subItem.href}
                                                    className="flex flex-col p-3 rounded-xl hover:bg-[#f5f5f7] transition-colors group"
                                                >
                                                    <span className="text-xs font-semibold text-[#1d1d1f]">
                                                        {subItem.title}
                                                    </span>
                                                    <span className="text-[11px] text-[#1d1d1f]/60 mt-0.5 font-sans">
                                                        {subItem.desc}
                                                    </span>
                                                </SafeLink>
                                            ))}
                    ...
                    <div className="flex items-center space-x-3 sm:space-x-4 rtl:space-x-reverse shrink-0">
                        ...
                        <a
                            ...
                            className="hidden sm:inline-flex p-2 text-[#1d1d1f]/70 hover:text-[#25D366] transition-colors"
                        ...
                        {auth?.user ? (
                            <SafeLink href={route('dashboard')}>
                                <button className="px-3.5 py-1.5 rounded-[980px] border border-black/10 bg-white text-[#1d1d1f] hover:bg-[#f5f5f7] text-[12px] font-medium transition-all shadow-sm">
                            ...
                            <div className="flex items-center space-x-3 rtl:space-x-reverse">
                                <SafeLink
                                    href={route('login')}
                                    className="text-[12px] font-medium text-[#1d1d1f]/75 hover:text-[#1d1d1f] transition-colors px-2"
                                >
                        ...
                        <button
                            ...
                            className="lg:hidden p-2 text-[#1d1d1f]/70 hover:text-[#1d1d1f] transition-colors"
                ...
                {/* Mobile Drawer */}
                {mobileMenuOpen && (
                    <div className="lg:hidden border-b border-black/5 bg-white/95 backdrop-blur-xl px-6 py-5 space-y-3">
                        <div className="flex flex-col space-y-2 text-sm text-[#1d1d1f]">
                            {navItems.map((item) => (
                                <SafeLink
                                    key={item.id}
                                    href={item.href}
                                    onClick={() => setMobileMenuOpen(false)}
                                    className="py-2 hover:text-[#0071e3] border-b border-black/5"
                                >
                                    {item.label}
                                </SafeLink>
                            ))}
                            {!auth?.user && (
                                <div className="pt-2 flex items-center gap-3">
                                    <SafeLink
                                        href="/login"
                                        onClick={() => setMobileMenuOpen(false)}
                                        className="flex-1 text-center py-2 rounded-xl border border-black/10 text-sm font-medium"
                                    >
                                        {__('general.sign_in') || 'Sign in'}
                                    </SafeLink>
                                    <SafeLink
                                        href="/start-project"
                                        onClick={() => setMobileMenuOpen(false)}
                                        className="flex-1 text-center py-2 rounded-xl bg-[#0071e3] text-white text-sm font-medium"
                                    >
            ...
            <footer className="w-full bg-[#f5f5f7] border-t border-black/5 py-16 px-6 sm:px-12 text-[12px] text-[#1d1d1f]/70">
                <div className="max-w-[1280px] mx-auto flex flex-col lg:flex-row justify-between gap-12">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 lg:gap-12 flex-1">
                        <div className="space-y-3">
                            <div className="text-[#1d1d1f] font-semibold tracking-tight">{__('general.contact_us') || 'Contact Us'}</div>
                            <ul className="space-y-2 text-[12px]">
                                <li><a href="https://wa.me/201015218548" target="_blank" rel="noopener noreferrer" className="hover:text-[#1d1d1f] transition-colors">{__('general.whatsapp_direct') || 'WhatsApp Direct'}</a></li>
                                <li><a href="mailto:admin@musoftwares.com" className="hover:text-[#1d1d1f] transition-colors">{__('general.email_studio') || 'Email Studio'}</a></li>
                        ...
                        <div className="space-y-3">
                            <div className="text-[#1d1d1f] font-semibold tracking-tight">{__('general.solutions') || 'Solutions'}</div>
                            <ul className="space-y-2 text-[12px]">
                                <li><SafeLink href="/platforms/erp" className="hover:text-[#1d1d1f] transition-colors">Enterprise ERP</SafeLink></li>
                                <li><SafeLink href="/platforms/crm" className="hover:text-[#1d1d1f] transition-colors">WhatsApp Cloud API</SafeLink></li>
                                <li><SafeLink href="/platforms/cloud" className="hover:text-[#1d1d1f] transition-colors">Meta Graph Suite</SafeLink></li>
                                <li><SafeLink href="/start-project" className="hover:text-[#1d1d1f] transition-colors">System Architecture Wizard</SafeLink></li>
                            </ul>
                        </div>

                        <div className="space-y-3">
                            <div className="text-[#1d1d1f] font-semibold tracking-tight">{__('general.press_center') || 'Press Center'}</div>
                            <ul className="space-y-2 text-[12px]">
                                <li><SafeLink href="/estimator" className="hover:text-[#1d1d1f] transition-colors">{__('general.estimator') || 'Architecture Estimator'}</SafeLink></li>
                                <li><SafeLink href="/portfolio" className="hover:text-[#1d1d1f] transition-colors">{__('general.portfolio') || 'Case Studies Archive'}</SafeLink></li>
                                <li><SafeLink href="/about/mahmoud-amin" className="hover:text-[#1d1d1f] transition-colors">{__('general.leadership_bio') || 'Leadership Bio (Mahmoud Amin)'}</SafeLink></li>
                                <li><SafeLink href="/compare/laravel-vs-nodejs" className="hover:text-[#1d1d1f] transition-colors">Tech Benchmarks (Laravel vs Node.js)</SafeLink></li>
                            </ul>
                        </div>

                        <div className="space-y-3">
                            <div className="text-[#1d1d1f] font-semibold tracking-tight">{__('general.legal') || 'Legal & Privacy'}</div>
                            <ul className="space-y-2 text-[12px]">
                                <li><SafeLink href="/privacy-policy" className="hover:text-[#1d1d1f] transition-colors">{__('general.privacy_policy') || 'Privacy Policy'}</SafeLink></li>
                                <li><SafeLink href="/terms-of-service" className="hover:text-[#1d1d1f] transition-colors">{__('general.terms_of_service') || 'Terms & SLA'}</SafeLink></li>
                                <li><span className="text-[#1d1d1f]/40">Security Architecture</span></li>
                                <li><span className="text-[#1d1d1f]/40">GDPR Compliance</span></li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-between items-start lg:items-end space-y-6">
                        <div className="flex items-center space-x-5 rtl:space-x-reverse text-[#1d1d1f]/70">
                            <a href="https://www.linkedin.com/in/musoftwareuno/?locale=ar" target="_blank" rel="noopener noreferrer" className="hover:text-[#1d1d1f] transition-colors">LinkedIn</a>
                            <a href="https://github.com/musoftware" target="_blank" rel="noopener noreferrer" className="hover:text-[#1d1d1f] transition-colors">GitHub</a>
                            <a href="https://x.com/MusoftwareUno" target="_blank" rel="noopener noreferrer" className="hover:text-[#1d1d1f] transition-colors">X</a>
                            <a href="https://www.facebook.com/musoftwares.com.page/" target="_blank" rel="noopener noreferrer" className="hover:text-[#1d1d1f] transition-colors">Facebook</a>
                        </div>

                        <div className="flex items-center space-x-2 rtl:space-x-reverse text-[#1d1d1f]/60 text-[11px]">
                ...
                <div className="max-w-[1280px] mx-auto mt-12 pt-6 border-t border-black/5 flex flex-col sm:flex-row items-center justify-between text-[#1d1d1f]/50 text-[11px]">
            ...
            <Dialog open={isGuestTicketOpen} onOpenChange={setIsGuestTicketOpen}>
                <DialogContent className="sm:max-w-md bg-white border-black/10 text-[#1d1d1f] rounded-[24px] shadow-2xl p-6">
                    <DialogHeader>
                        <DialogTitle className="text-[#1d1d1f] font-semibold text-lg">{__('general.submit_guest_ticket') || 'Connect with Engineering Studio'}</DialogTitle>
                        <DialogDescription className="text-[#1d1d1f]/60 text-xs">
                    ...
                    <form onSubmit={submitGuestTicket} className="space-y-4 text-xs">
                        <div className="space-y-1.5">
                            <Label htmlFor="name" className="text-[#1d1d1f]/80 font-medium">{__('general.name') || 'Name'}</Label>
                            <Input id="name" required value={data.name} onChange={e => setData('name', e.target.value)} className="bg-[#f5f5f7] border-black/10 text-[#1d1d1f] rounded-xl focus:bg-white" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="email" className="text-[#1d1d1f]/80 font-medium">{__('general.email') || 'Email'}</Label>
                            <Input id="email" type="email" required value={data.email} onChange={e => setData('email', e.target.value)} className="bg-[#f5f5f7] border-black/10 text-[#1d1d1f] rounded-xl focus:bg-white" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="mobile" className="text-[#1d1d1f]/80 font-medium flex items-center gap-2">
                                {__('general.mobile') || 'Mobile / WhatsApp'} <MessageCircle className="h-3.5 w-3.5 text-[#25D366]" />
                            </Label>
                            <Input id="mobile" required value={data.mobile} onChange={e => setData('mobile', e.target.value)} className="bg-[#f5f5f7] border-black/10 text-[#1d1d1f] rounded-xl focus:bg-white" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="body" className="text-[#1d1d1f]/80 font-medium">{__('general.message') || 'Scope Brief'}</Label>
                            <Textarea id="body" required value={data.body} onChange={e => setData('body', e.target.value)} rows={4} className="bg-[#f5f5f7] border-black/10 text-[#1d1d1f] rounded-xl focus:bg-white" />
                        </div>
                        <DialogFooter className="pt-2">
                            <button type="button" onClick={() => setIsGuestTicketOpen(false)} className="px-4 py-2 rounded-full border border-black/10 bg-white text-[#1d1d1f] hover:bg-[#f5f5f7] text-xs font-medium">
