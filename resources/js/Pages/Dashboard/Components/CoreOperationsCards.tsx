...
export default function CoreOperationsCards({ stats, safeRoute }: CoreOperationsCardsProps) {
    const { post, processing } = useForm<{ amount?: number }>({});
                ...
                <div className="mt-auto pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {[50, 100, 250, 500].map((amount) => (
                            <button 
                                ...
                                disabled={processing}
                                className="flex flex-col items-center justify-center p-2 rounded-lg bg-slate-50 border border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 hover:text-emerald-700 transition-colors disabled:opacity-50"
                            >
                    ...
                    <div className="mt-3 text-center">
                        <Link href={safeRoute('financial.add-balance', undefined, '/financial/add-balance')} className="text-xs font-medium text-slate-500 hover:text-slate-900 flex items-center justify-center">
                            {__('general.custom_amount')}<ArrowRight className="w-3 h-3 ms-1 rtl:rotate-180" />
                        </Link>
                    </div>
                </div>
            </div>
