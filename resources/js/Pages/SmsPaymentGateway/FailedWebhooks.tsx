...
export default function FailedWebhooks({ failedWebhooks }: FailedWebhooksProps) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">{__('general.failed_webhooks')}</h2>}>
            <Head title={__('general.failed_webhooks')} />
            ...
            <div className="py-8 md:py-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                                ...
                                {__('general.failed_webhooks')}</h1>
                            <p className="text-slate-500 mt-1">{__('general.review_the_details_and_payloads_of_faile')}</p>
                        </div>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>{__('general.delivery_failures_log')}</CardTitle>
                            <CardDescription>
                                ...
                                <Table>
                                    <TableHeader className="bg-slate-50">
                                        <TableRow>
                                            ...
                                            <TableHead>{__('general.target_url')}</TableHead>
                                            <TableHead>{__('general.error_message')}</TableHead>
                                        ...
                                        {(failedWebhooks.data as any).length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={4} className="text-center py-12 text-slate-500">
                                                    {__('general.no_failed_webhooks_found')}</TableCell>
                                            </TableRow>
                                        ) : (
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
                            <DialogHeader>
                                <DialogTitle>{__('general.webhook_failure_details')}</DialogTitle>
                            </DialogHeader>
                            {selectedWebhook && (
                                <div className="space-y-6 mt-4">
                                    <div className="space-y-2">
                                        <h4 className="font-semibold text-sm text-slate-900">{__('general.failed_at')}</h4>
                                    ...
                                    <div className="space-y-2">
                                        <h4 className="font-semibold text-sm text-slate-900">{__('general.target_url')}</h4>
                                    ...
                                    <div className="space-y-2">
                                        <h4 className="font-semibold text-sm text-rose-600">{__('general.error_message')}</h4>
                                    ...
                                    <div className="space-y-2">
                                        <h4 className="font-semibold text-sm text-slate-900">{__('general.payload_sent')}</h4>
