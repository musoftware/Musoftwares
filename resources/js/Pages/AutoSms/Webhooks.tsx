...
export default function Webhooks({ webhook }: WebhooksProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('autosms.webhook.update'));
    };

    const handleDelete = () => {
        if (webhook && confirm('Are you sure you want to remove this webhook?')) {
            router.delete(route('autosms.webhook.delete', webhook.id));
        }
    };

    const handleTest = () => {
        setTesting(true);
        router.post(route('autosms.webhook.test'), {}, {
                    ...
                    <Card>
                        <form onSubmit={handleSubmit}>
                            <CardHeader>
                                ...
                                <CardDescription>Configure where AutoSMS should send HTTP POST requests when a transaction is detected.</CardDescription>
                            </CardHeader>
