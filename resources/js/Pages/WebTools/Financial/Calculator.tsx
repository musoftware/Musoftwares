...
export default function Calculator({ input, result, amount_to_pay, calculation_type, ninety_percent, auth }: Props) {
    ...
    return (
        <WebToolsLayout title={__('general.payment_calculator')} activeNav="explore">
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6">
                <div className="mb-8">
                    ...
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">{__('general.payment_calculator')}</h1>
                    <p className="mt-2 text-lg text-slate-600">
                        {__('general.calculate_transfer_fees_and_process_paym')}</p>
                </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <Card className="bg-blue-50/50 border-blue-100 shadow-sm">
                            <CardContent className="p-4 flex gap-4">
                                ...
                                <div>
                                    <h3 className="font-semibold text-blue-900 mb-1">{__('general.choose_your_payment_method')}</h3>
                                    ...
                                    <ul className="text-sm text-blue-800 list-disc ps-5 space-y-1">
                                        <li><strong>Visa/Master:</strong> {__('general.pay_with_credit_card_to_receive_funds')}</li>
                                        <li><strong>Mobile Wallet:</strong> {__('general.pay_from_mobile_wallet_to_receive_funds')}</li>
                                    </ul>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <div className="space-y-4">
                            <Label className="text-base font-semibold">{__('general.calculation_type')}</Label>
                            <RadioGroup value={calcType} onValueChange={(val: any) => setCalcType(val)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                >
                                    ...
                                    <div>
                                        <div className="font-semibold text-slate-900 flex items-center gap-2">
                                            <Smartphone className="w-4 h-4 text-slate-500" /> {__('general.mobile_wallet')}</div>
                        ...
                        <div className="space-y-2 pt-4">
                            ...
                            <p className="text-sm text-slate-500">{__('general.enter_the_amount_you_wish_to_withdraw_or')}</p>
                        </div>
                            ...
                            <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border rounded-xl overflow-hidden shadow-sm">
                                    <div className="bg-white p-6 border-b md:border-b-0 md:border-e border-slate-100">
                                        <p className="text-sm font-medium text-slate-500 mb-1">{__('general.you_receive')}</p>
                                    ...
                                    <div className="bg-white p-6 border-b md:border-b-0 md:border-e border-slate-100">
                                        <p className="text-sm font-medium text-slate-500 mb-1">{__('general.amount_to_pay')}</p>
                                        <div className="flex items-baseline gap-1">
                                ...
                                <div className="mt-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
                                    ...
                                    <p className="text-sm text-slate-600">
                                        {__('general.pay')}<strong>{formatNumber(amountToPay)} EGP</strong> {calcType === 'visa_master' ? 'with your Visa/Master card' : 'from your mobile wallet'} to receive <strong>{formatNumber(receiveAmount)} EGP</strong> in your account.
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="pt-8">
                            <h3 className="font-bold text-xl mb-4">{__('general.how_it_works')}</h3>
                                ...
                                <CardContent className="pt-6">
                                    <div className="text-center mb-6">
                                        {isLoggedIn ? (
                                            <>
                                                ...
                                                <p className="text-sm text-slate-500 mt-1">{__('general.your_account_is_ready_for_instant_proces')}</p>
                                            </>
                                        ) : (
                                            <>
                                                <h4 className="font-semibold text-slate-900">{__('general.payment_calculator')}</h4>
                                                <p className="text-sm text-slate-500 mt-1">{__('general.calculate_fees_and_explore_payment_optio')}</p>
                                            </>
                                        )}
                                    </div>
                                    ...
                                    {isLoggedIn && (
                                        <div className="mb-6 space-y-2">
                                            <Label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{__('general.quick_link')}</Label>
                                        ...
                                        {isLoggedIn ? (
                                            <>
                                                <Button className="w-full h-12 text-base font-semibold" disabled={!signedPayUrl} onClick={() => window.location.href = signedPayUrl}>
                                                    <CreditCard className="w-5 h-5 me-2" /> {__('general.secure_checkout')}</Button>
                                                <Button variant="outline" className="w-full" onClick={() => router.get(route('dashboard'))}>
                                                    {__('general.back_to_dashboard')}</Button>
                                            </>
                                        ) : (
                                            <>
                        ...
                        <Card>
                            <CardHeader className="pb-3">
                                <CardTitle className="text-sm flex items-center gap-2">
                                    <Info className="w-4 h-4" /> {__('general.fee_structure')}</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-2">
                                {calcType === 'visa_master' ? (
                                    ...
                                    <div className="space-y-3">
                                        <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider">{__('general.mobile_wallet_transfer')}</h5>
                                        <ul className="text-sm text-slate-600 space-y-2">
