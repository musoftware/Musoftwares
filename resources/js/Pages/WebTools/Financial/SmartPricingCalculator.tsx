...
export default function SmartPricingCalculator() {
    ...
    return (
        <WebToolsLayout title={__('general.smart_pricing_calculator')} activeNav="explore">
            <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6">
                <div className="mb-8">
                    <span className="inline-block px-3 py-1 rounded-full bg-indigo-100 text-indigo-700 text-sm font-medium mb-3">
                        {__('general.ecommerce_tool')}</span>
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
                        <Calculator className="w-8 h-8 text-indigo-500" />
                        {__('general.smart_pricing_calculator')}</h1>
                    <p className="mt-2 text-lg text-slate-600">
                        {__('general.calculate_optimal_product_pricing_consid')}</p>
                </div>
                        ...
                        <Card className="border-slate-200 shadow-sm">
                            <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
                                <CardTitle className="text-lg flex items-center gap-2">
                                    ...
                                    {__('general.product_marketing')}</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-6 space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <Label className="text-slate-600">{__('general.product_cost')}</Label>
                        ...
                        <Card className="border-slate-200 shadow-sm">
                            <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
                                <CardTitle className="text-lg flex items-center gap-2">
                                    ...
                                    {__('general.operations_logistics')}</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-6 space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-slate-600">{__('general.monthly_expenses')}</Label>
                                        <div className="relative">
                                            <span className="absolute start-3 top-2.5 text-slate-400">$</span>
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-slate-600">{__('general.confirmation_rate')}</Label>
                                        <div className="relative">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-slate-600">{__('general.delivery_rate')}</Label>
                                        <div className="relative">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-slate-600">{__('general.shipping_cost')}</Label>
                                        <div className="relative">
                                            <span className="absolute start-3 top-2.5 text-slate-400">$</span>
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-slate-600">{__('general.return_cost')}</Label>
                                        <div className="relative">
                                            <span className="absolute start-3 top-2.5 text-slate-400">$</span>
                        ...
                        {results ? (
                            <div className="space-y-6">
                                <Card className="bg-indigo-600 text-white border-none shadow-md">
                                    <CardContent className="p-8">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div>
                                                <p className="text-indigo-100 font-medium mb-1">{__('general.breakeven_point')}</p>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <p className="text-indigo-200 text-sm mb-1">{__('general.total_unit_cost')}</p>
                                                ...
                                                <div className="col-span-2">
                                                    <p className="text-indigo-200 text-sm mb-1">{__('general.projected_delivered')}</p>
                                ...
                                <h3 className="text-xl font-bold text-slate-900 mt-8 mb-4 flex items-center gap-2">
                                    ...
                                    {__('general.pricing_suggestions')}</h3>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {results.suggestions.map((s, i) => (
                                        <Card key={i} className={`border-2 ${i === 2 ? 'border-emerald-500 shadow-emerald-100' : 'border-slate-200'} shadow-sm relative overflow-hidden`}>
                                            {i === 2 && (
                                                <div className="absolute top-0 end-0 bg-emerald-500 text-white text-[10px] font-bold px-2 py-1 rounded-bs-lg uppercase tracking-wider">
                                                    {__('general.recommended')}</div>
                                            )}
                                            <CardContent className="p-5">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div>
                                                        <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">{__('general.target_margin')}</p>
                                                        <p className="text-2xl font-black text-slate-900">{s.margin}%</p>
                                                    </div>
                                                    <div className="text-end">
                                                        <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">{__('general.sell_price')}</p>
                                                ...
                                                <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                                                    ...
                                                    <div className="text-end">
                                                        <p className="text-xs text-slate-500 mb-1">{__('general.monthly_net_profit')}</p>
                            ...
                            <Card className="border-slate-200 shadow-sm h-full flex flex-col items-center justify-center p-12 text-center text-slate-500">
                                ...
                                <h3 className="text-xl font-medium text-slate-700">{__('general.waiting_for_valid_inputs')}</h3>
                                <p className="mt-2">{__('general.enter_your_product_and_marketing_costs_t')}</p>
                            </Card>
                        )}
                    </div>
                </div>
