        ...
        <div className="min-h-[calc(100vh-theme(spacing.14))] bg-slate-50 font-sans flex flex-col">
            <div className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10 shrink-0">
                <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-bold text-sm text-slate-800 tracking-tight">{__('general.domain_analytics')}</span>
                </div>
                <div className="flex items-center gap-3">
                    <Button variant="outline" onClick={exportCsv} disabled={results.length === 0} className="h-8 text-xs font-bold gap-1.5">
                        <Download className="w-3.5 h-3.5" />{__('general.export_report')}</Button>
                ...
                <div className="w-80 bg-white border-r border-slate-200 flex flex-col shrink-0 overflow-y-auto">
                    <div className="p-5 space-y-6">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">{__('general.enter_domains_to_perform_deep_reconnaissance_one_per_line')}</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {status === 'running' ? (
                                ...
                                >
                                    <Square className="w-4 h-4 fill-current" />{__('general.stop_scan')}</Button>
                            ) : (
                                <Button
                                    onClick={handleStart}
                                ...
                                >
                                    <Play className="w-4 h-4 fill-current" />{__('general.launch_scan')}</Button>
                            )}
                    ...
                    <div className="flex-1 p-6 max-w-6xl mx-auto w-full">
                        {results.length === 0 && status !== 'running' ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
                                ...
                                <p className="text-sm font-medium">{__('general.enter_domains_to_generate_an_intelligence_report')}</p>
                            </div>
                        ) : (
                                    ...
                                    <div key={index} className="space-y-6 animate-in fade-in duration-500 pt-6 first:pt-0 border-t-2 first:border-t-0 border-slate-200 border-dashed">
                                        ...
                                        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex items-start justify-between gap-6">
                                            <div>
                                                ...
                                                <p className="text-sm text-slate-500 font-medium mt-1">{__('general.intelligence_report_generated_successfully')}</p>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                                                <div className="bg-slate-50 border-b border-slate-100 px-5 py-3.5 flex items-center gap-2">
                                                    ...
                                                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">{__('general.whois_registration')}</h3>
                                                </div>
                                                <div className="p-5 space-y-4 flex-1">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.creation_date')}</p>
                                                        ...
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.expiry_date')}</p>
                                                    ...
                                                    <div>
                                                        <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.name_servers')}</p>
                                                        <div className="space-y-1">
                                                            ...
                                                            )) : <p className="text-sm text-slate-500">{__('general.not_found_1')}</p>}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                                                <div className="bg-slate-50 border-b border-slate-100 px-5 py-3.5 flex items-center gap-2">
                                                    ...
                                                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">{__('general.dns_records')}</h3>
                                                </div>
                                                <div className="p-5 space-y-4 flex-1 max-h-[300px] overflow-y-auto">
                                                    ...
                                                    <div>
                                                        <Badge variant="outline" className="mb-2 text-[10px] font-bold">{__('general.txt_records')}</Badge>
                                                        <div className="space-y-1">
                                                    ...
                                                    <div>
                                                        <Badge variant="outline" className="mb-2 text-[10px] font-bold">{__('general.cname_records')}</Badge>
                                                        <div className="flex flex-wrap gap-2">
                                            ...
                                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden md:col-span-2">
                                                <div className="bg-slate-50 border-b border-slate-100 px-5 py-3.5 flex items-center gap-2">
                                                    ...
                                                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">{__('general.on_page_analysis')}</h3>
                                                </div>
                                                <div className="p-5">
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.page_title')}</p>
                                                            <p className="text-sm font-medium text-slate-900 line-clamp-2">{res.seo?.title || 'No Title Found'}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.meta_description')}</p>
                                                            <p className="text-xs text-slate-600 line-clamp-3 leading-relaxed">{res.seo?.description || 'No Description Found'}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.meta_keywords')}</p>
                                                            <p className="text-xs text-slate-600 line-clamp-2">{res.seo?.keywords || '-'}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1">{__('general.server_header')}</p>
