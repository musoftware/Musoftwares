...
function DialectBadge({ dialect }: { dialect: string }) {
    ...
    return <span className="text-[10px] font-bold bg-indigo-500/15 text-indigo-400 border border-indigo-500/25 px-2 py-0.5 rounded-full">{map[dialect] ?? dialect}</span>;
}
    ...
    return (
        <div className={`border rounded-2xl p-5 transition-all ${agent.active ? 'border-indigo-500/30 bg-indigo-500/5' : 'border-slate-800 bg-slate-900'}`}>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
                ...
                <span className="text-[10px] font-bold bg-slate-800 text-slate-400 border border-slate-700 px-2 py-0.5 rounded-full">{agent.tone ?? 'Professional'}</span>
            </div>
            ...
            <button onClick={onEdit} className="w-full py-2 bg-slate-800 border border-slate-700 text-slate-400 rounded-xl text-xs font-bold hover:bg-slate-700 transition-all flex items-center justify-center gap-1.5">
            ...
            <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5 my-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="Sales Bot, Support Agent..."
                            className="w-full px-3 py-2.5 text-sm bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-xl outline-none text-white" />
                    </div>
                    <div>
                        ...
                        <input value={number} onChange={e => setNumber(e.target.value)} placeholder="+962 7..."
                            className="w-full px-3 py-2.5 text-sm bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-xl outline-none text-white font-mono" />
                    </div>
                    <div>
                        <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 block mb-1.5">Arabic Dialect</label>
                        <select value={dialect} onChange={e => setDialect(e.target.value)} className="w-full px-3 py-2.5 text-sm bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-xl outline-none text-white">
                    ...
                    <div>
                        ...
                        <select value={tone} onChange={e => setTone(e.target.value)} className="w-full px-3 py-2.5 text-sm bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-xl outline-none text-white">
                ...
                <div>
                    ...
                    <textarea value={context} onChange={e => setContext(e.target.value)} rows={3} placeholder="Describe your business, products, pricing, and FAQs. The AI uses this to answer customer questions..."
                        className="w-full px-3 py-2.5 text-sm bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-xl outline-none text-white resize-none" />
                </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input value={apiKey} onChange={e => setApiKey(e.target.value)} type="password" placeholder="sk-..."
                            className="w-full pl-9 pr-4 py-2.5 text-sm bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-xl outline-none text-white font-mono" />
                    </div>
                </div>
                ...
                <div className="flex gap-3 pt-2 border-t border-slate-800">
                    ...
                    <button onClick={() => { if (!name.trim() || !number.trim()) return; onAdd({ name, number, dialect, tone, context, goal, apiKey, active: false, conversations_today: 0, qualified_leads: 0, response_rate: 0 }); onClose(); }}
                        ...
                        className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-500 transition-all disabled:opacity-40">
                        Deploy Agent
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-slate-950 font-sans text-slate-100">
            ...
            <div className="h-14 bg-slate-900/80 backdrop-blur border-b border-slate-800 flex items-center justify-between px-6 sticky top-0 z-10">
                ...
                <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-500/20 active:scale-95">
            ...
            <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
                ...
                {agents.length > 0 ? (
                    ...
                    <div className="py-24 text-center border border-dashed border-slate-800 rounded-2xl">
                        ...
                        <button onClick={() => setShowAdd(true)} className="mt-6 flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-500 transition-all mx-auto shadow-lg shadow-indigo-500/20">
