...
export const ProjectProgressBar: React.FC<ProjectProgressBarProps> = ({
    ...
    return (
        <section className="w-full bg-[#141416]/95 border border-white/10 rounded-2xl p-6 sm:p-7 shadow-2xl backdrop-blur-2xl transition-all">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/5">
                ...
                <div className="flex items-baseline sm:items-end gap-3 self-start sm:self-auto">
                    <div className="text-left sm:text-right">
                        ...
                        <div className="text-3xl font-mono font-bold text-white tracking-tight tabular-nums mt-0.5">
                            {percentage}<span className="text-sm font-sans font-normal text-zinc-400">%</span>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div
                                ...
                                className={`group p-3.5 rounded-xl border transition-all duration-300 ${
                                    isDone
                                        ...
                                        : isCurrent
                                        ? 'border-white/20 bg-gradient-to-b from-zinc-800/60 to-zinc-900/90 text-white shadow-lg ring-1 ring-white/10'
                                        : 'border-white/5 bg-zinc-950/40 text-zinc-600'
                                }`}
                            >
                                <div className="flex items-center justify-between text-[11px] font-mono mb-2">
                                    <span className="text-zinc-400 font-medium">{stg.num}</span>
                                    <span className="flex items-center gap-1.5">
                                        {isDone && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />}
                                        {isCurrent && <Clock className="w-3.5 h-3.5 text-sky-400 animate-pulse" />}
                                        {!isDone && !isCurrent && <Circle className="w-3 h-3 text-zinc-700" />}
                                        <span className="text-[10px] uppercase font-mono tracking-wider text-zinc-400">
                                            {isDone ? 'Delivered' : isCurrent ? 'Active' : 'Queued'}
                                        </span>
                                    </span>
                                </div>
                                <div className="text-xs font-semibold text-white tracking-tight">{stg.title}</div>
                                <div className="text-[11px] text-zinc-400 mt-0.5 line-clamp-1">{stg.desc}</div>
                            </div>
                        );
                    })}
                </div>
            ...
            {!project.is_brief_complete && (
                <div className="mt-6 p-4 rounded-xl bg-gradient-to-r from-amber-500/10 via-zinc-900/80 to-zinc-900/80 border border-amber-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <button
                        ...
                        className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-white text-black hover:bg-zinc-200 transition-colors shrink-0 shadow-sm"
                    >
            ...
            {showBriefForm && (
                <form onSubmit={handleBriefSubmit} className="mt-4 p-4 rounded-xl bg-zinc-950 border border-white/10 space-y-3 animate-in fade-in duration-200">
                    ...
                    <textarea
                        ...
                        className="w-full text-xs p-3.5 rounded-lg bg-zinc-900/90 border border-zinc-700 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-white/30 font-mono leading-relaxed"
                    />
                    <div className="flex justify-end gap-2.5 pt-1">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold bg-emerald-500 text-black hover:bg-emerald-400 disabled:opacity-40 transition-colors"
                        >
            ...
            {briefSuccessMsg && (
                <div className="mt-3 text-xs text-emerald-400 font-mono font-medium">
                    {briefSuccessMsg}
                </div>
            )}
            ...
            {milestones.length > 0 && (
                <div className="mt-6 pt-5 border-t border-white/5">
                    <div className="flex items-center justify-between mb-3 text-[11px] font-mono uppercase text-zinc-400">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {milestones.map((m) => (
                            ...
                            >
                                <span className={m.is_completed ? 'line-through text-zinc-500' : 'text-zinc-200'}>
                                    {m.title}
                                </span>
