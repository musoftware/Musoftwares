...
export default function Index({ clients }: { clients: any[] }) {
    ...
    return (
        <ERPLayout title={__("general.referrals")} workspaceName={workspaceName} tenantId={tenantId} menuItems={menuItems} lockedAddons={lockedAddons}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6 dashboard-container at-mobile-scroll-fix">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">{__("erp.client_referrals")}</h1>
                        <p className="text-slate-500 text-sm mt-1">{__("erp.manage_referral_links_and_track")}</p>
                    </div>
                ...
                <Card className="bg-white border border-slate-200 shadow-sm overflow-hidden">
                    <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-4">
                        <CardTitle className="text-slate-900 text-sm font-semibold">{__("erp.clients_referral_links")}</CardTitle>
                    </CardHeader>
                    <CardContent className="p-0">
                        {clients && clients.length > 0 ? (
                                ...
                                <table className="w-full text-start text-sm border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                            <th className="px-6 py-3">{__("erp.client")}</th>
                                            <th className="px-6 py-3">{__("general.referred_by")}</th>
                                            <th className="px-6 py-3">{__("general.referral_link")}</th>
                                            <th className="px-6 py-3 text-end">{__("general.actions")}</th>
                                        </tr>
                                    </thead>
                            ...
                            <EmptyState 
                                icon={LinkIcon} 
                                title={__("erp.no_clients_found")} 
                                description={__("erp.clients_and_their_referral_networks")} 
