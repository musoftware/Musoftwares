...
export default function Show({ campaign }) {
    ...
    const handleSaveContent = () => {
        setIsSaving(true);
        router.put(route('admin.campaigns.update', campaign.id), contentForm, {
    ...
    const handleGenerateAI = async () => {
        ...
        try {
            const response = await window.axios.post(route('admin.campaigns.generate-ai'), aiForm);
    ...
    const handleSchedule = () => {
        if(confirm('Ready to schedule this campaign? (It will be sent by the background processor)')) {
            router.post(route('admin.campaigns.schedule', campaign.id));
        }
    };

    const handlePause = () => {
        router.post(route('admin.campaigns.pause', campaign.id));
    };

    const handleResume = () => {
        router.post(route('admin.campaigns.resume', campaign.id));
    };

    const isEditable = campaign.status === 'draft';

    return (
        <AdminSidebarLayout title={`Campaign: ${campaign.name}`} header="Campaign Builder">
