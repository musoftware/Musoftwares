...
import { Calculator, Save, FileText, CheckCircle2, Play, Loader2 } from 'lucide-react';
...
export default function Index({ proposals }) {
    ...
    const handleCalculate = async () => {
        ...
        try {
            // Using axios since Inertia would do a full navigation cycle on POST which is harder to handle JSON response natively
            const response = await window.axios.post(route('admin.calculator.calculate-ai'), {
    ...
    const handleSaveProposal = () => {
        ...
        router.post(route('admin.calculator.save'), {
    ...
    const handleConvertToContract = (id) => {
        if (confirm('Are you sure you want to convert this proposal into a Contract?')) {
            router.post(route('admin.calculator.convert', id));
        }
    };

    return (
        <AdminSidebarLayout title="Price Calculator" header="Smart Price Calculator">
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                {proposals.length > 0 ? proposals.map(proposal => (
                                    <div key={proposal.id} className="flex flex-col p-3 border rounded-lg bg-gray-50">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <div className="font-medium text-sm">{proposal.project_name}</div>
                                                <div className="text-xs text-gray-500">{new Date(proposal.created_at).toLocaleDateString()}</div>
                                            </div>
                                        ...
                                        {proposal.status === 'converted_to_contract' ? (
                                            ...
                                            <div className="mt-2 flex space-x-2">
                                                <Button size="sm" variant="outline" className="w-full text-xs" onClick={() => handleConvertToContract(proposal.id)}>
                                                    Convert to Contract
                                                </Button>
                                            </div>
                                        )}
                ...
                <div className="lg:col-span-2">
                    <Card className="h-full min-h-[600px]">
                        <CardHeader className="flex flex-row items-center justify-between">
                            ...
                            {aiResult && (
                                <Button onClick={handleSaveProposal} disabled={isSaving} variant="default" className="bg-green-600 hover:bg-green-700">
                                ...
                                <div className="space-y-6">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-4 rounded-lg border bg-blue-50/50">
                                            ...
                                            <div className="text-2xl font-bold text-gray-900">{aiResult.parsed_data.total_duration_days} Days</div>
                                        </div>
                                        <div className="p-4 rounded-lg border bg-green-50/50">
                                            <div className="text-sm text-gray-500 font-medium">Estimated Cost</div>
                                            <div className="text-2xl font-bold text-green-700">{aiResult.total_cost_egp.toLocaleString()} EGP</div>
                                        </div>
                                    </div>
                                    
                                    <div>
                                        <h3 className="font-semibold text-lg mb-2">Proposal Document</h3>
