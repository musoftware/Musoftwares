...
export default function Edit({ exchange, currencies }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        date_string: exchange.date_string,
        currency1: String(exchange.currency1),
        currency2: String(exchange.currency2),
        rate: String(exchange.rate),
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.currency-exchanges.update', exchange.id));
    };

    return (
        <AdminSidebarLayout
            title={__('admin.edit_currency_exchange')}
            header={__('admin.edit_currency_exchange')}
        >
            <Head title={__('admin.edit_currency_exchange')} />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900">
                            {__('admin.edit_currency_exchange')}
                        </h1>
                    </div>
                </div>
