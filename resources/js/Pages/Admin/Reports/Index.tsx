...
import {
  ...
  BarChart3, Briefcase, Ticket, UserPlus, ArrowUpRight, ArrowDownRight, Clock } from
'lucide-react';
...
import {
  ...
  CartesianGrid, Tooltip as RechartsTooltip, Legend, ResponsiveContainer } from
'recharts';
    ...
    <AdminSidebarLayout
      ...
      user={auth?.user}>
            ...
            <div className="space-y-6 pb-10">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-end gap-4 gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <Tabs defaultValue="overview" className="w-full space-y-6">
                  ...
                  <li key={source} className="flex justify-end gap-4 items-center border-b border-slate-100 pb-2">
                                            <span className="me-auto text-slate-600">{source}</span>
                                            <span className="font-semibold">{maskValue(formatCurrency(amount, businessCurrency))}</span>
                                        </li>
                  )}
                                    <li className="flex justify-end gap-4 items-center pt-2 text-base font-bold text-slate-900">
                                        <span className="me-auto">{__('general.total_income')}</span>
                  ...
                  <li key={type} className="flex justify-end gap-4 items-center border-b border-slate-100 pb-2">
                                            <span className="me-auto text-slate-600">{type}</span>
                                            <span className="font-semibold">{maskValue(formatCurrency(amount, businessCurrency))}</span>
                                        </li>
                  )}
                                    <li className="flex justify-end gap-4 items-center pt-2 text-base font-bold text-slate-900">
                                        <span className="me-auto">{__('general.total_expenses')}</span>
                        ...
                        <div className="bg-slate-900 text-white p-6 rounded-xl shadow flex items-center justify-end gap-4">
                            <span className="me-auto text-xl font-bold">{__('general.net_profit')}</span>
