...
import { ChevronLeft, FileText, AlertCircle, Calendar, ListChecks, Megaphone } from 'lucide-react';
...
export default function Edit({ project, report, types, priorities }: Props) {
    ...
    const [form, setForm] = useState({
        ...
        notify_client: report.notify_client,
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.put(
            route('admin.projects.reports.update', { project: project.id, report: report.id }),
            payload,
            {
    ...
    return (
        <AdminSidebarLayout
            title={`${__('general.edit_report')} · ${project.name}`}
            header={`${project.name} — ${__('general.reports')}`}
        >
            <Head title={`${__('general.edit_report')} · ${project.name}`} />
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            {pageErrors.title && (
                                <p className="text-xs text-rose-600">{pageErrors.title}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {pageErrors.summary && (
                                <p className="text-xs text-rose-600">{pageErrors.summary}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            {pageErrors.body && (
                                <p className="text-xs text-rose-600">{pageErrors.body}</p>
                            )}
                        </div>
                    </section>
                    ...
                    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                ...
                                {pageErrors.type && (
                                    <p className="text-xs text-rose-600">{pageErrors.type}</p>
                                )}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {pageErrors.priority && (
                                    <p className="text-xs text-rose-600">{pageErrors.priority}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                                ...
                                />
                                {pageErrors.period_start && (
                                    <p className="text-xs text-rose-600">{pageErrors.period_start}</p>
                                )}
                            </div>
                            <div className="space-y-2">
                                ...
                                />
                                {pageErrors.period_end && (
                                    <p className="text-xs text-rose-600">{pageErrors.period_end}</p>
                                )}
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="space-y-2">
                            ...
                            {pageErrors.published_at && (
                                <p className="text-xs text-rose-600">{pageErrors.published_at}</p>
                            )}
                        </div>
                    ...
                    <div className="flex items-center justify-end gap-2">
                        ...
                        <Button type="submit" disabled={submitting} className="gap-2 bg-emerald-600 hover:bg-emerald-700">
                            {submitting ? __('general.saving') : __('general.save_changes')}
                        </Button>
                    </div>
                </form>
            </div>
