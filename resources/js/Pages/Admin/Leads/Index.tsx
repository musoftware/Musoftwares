...
import { MoreHorizontal, Trash2, Edit2, Mail, Phone } from 'lucide-react';
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
export default function Index({ leads, currentTab }) {
    const handleStatusUpdate = (id, status) => {
        router.post(route('admin.leads.update-status', id), { status });
    };
    ...
    return (
        <AdminSidebarLayout title="Platform Leads" header="Leads CRM">
            <div className="mb-6 flex items-center justify-between">
                <div className="flex space-x-4">
                    <Link
                        href={route('admin.leads.index', { status: 'all' })}
                    ...
                    <Link
                        ...
                        className={`rounded-md px-4 py-2 text-sm font-medium ${currentTab === 'new' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                    >
                        New
                    </Link>
                    <Link
                        href={route('admin.leads.index', { status: 'contacted' })}
                        className={`rounded-md px-4 py-2 text-sm font-medium ${currentTab === 'contacted' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                    >
                        Contacted
                    </Link>
                    <Link
                        href={route('admin.leads.index', { status: 'converted' })}
                        className={`rounded-md px-4 py-2 text-sm font-medium ${currentTab === 'converted' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                    >
                        Converted
                    </Link>
                </div>
