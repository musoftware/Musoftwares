...
export default function Edit({ quotation, currencies }: EditProps) {
    ...
    const submit = (e: React.FormEvent) => {
        ...
        put(`/admin/marketplace/quotations/${quotation.id}`, {
            onSuccess: () => toast.success(__('quotations.updated_success')),
            onError: () => toast.error(__('validation.required')),
        });
    };
    ...
    return (
        <AdminSidebarLayout header={__('quotations.edit_title', { number: quotation.quotation_number })}>
            <Head title={`${__('quotations.edit_title', { number: quotation.quotation_number })} - ${__('quotations.admin_title')}`} />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm sticky top-4 z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-xl font-bold text-slate-900">{__('quotations.save_changes')}</h1>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button type="submit" disabled={processing} className="bg-slate-900 text-white hover:bg-slate-800 gap-2 px-6">
                            ...
                            {processing ? __('common.saving') : __('quotations.save_changes')}
                        </Button>
                    </div>
                </div>
