...
export default function View({ income, transactions, upcomingSchedule, total_stat }) {
    ...
    const handleDelete = () => {
        if (confirm('Are you sure you want to delete this recurring income?')) {
            router.delete(route('admin.recurring_income.delete', income.id));
        }
    };
    ...
    const handleDeleteWithTransactions = () => {
        if (confirm('Are you sure you want to delete this recurring income AND all its generated transactions? This cannot be undone.')) {
            router.delete(route('admin.recurring_income.delete_with_transaction', income.id));
        }
    };

    return (
        <AdminSidebarLayout title={`${income.title} - Recurring Details`} header="Business Operations">
            <Head title={`View Recurring Income - ${income.title}`} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-xl border shadow-sm md:col-span-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <span className="text-sm font-bold text-slate-900">{parseFloat(income.amount).toLocaleString()} {income.currency} ({income.currency_symbol})</span>
                        </div>
                        <div className="mt-2">
                            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Recurrence Pattern</span>
                            <span className="text-sm font-medium text-slate-800 capitalize">{income.details}</span>
                        </div>
                        <div className="mt-2">
                            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Category / Reason</span>
                            <span className="text-sm font-medium text-slate-800 bg-slate-100 border px-2 py-0.5 rounded inline-block mt-0.5">{income.reason || 'None'}</span>
                        </div>
                        <div className="mt-2">
                            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Start Date</span>
                            <span className="text-sm font-medium text-slate-800">{new Date(income.start_date).toLocaleDateString()}</span>
                        </div>
                        <div className="mt-2">
                ...
                <div className="bg-white p-6 rounded-xl border shadow-sm flex flex-col justify-between">
                    ...
                    <div className="mt-4 pt-4 border-t text-xs text-gray-400 flex items-start gap-1">
                        ...
                        <span>The cumulative revenue is calculated based on generated transactions associated with this schedule.</span>
                    </div>
                </div>
            </div>
                        ...
                        <div>
                            {transactions.length === 0 ? (
                                <div className="text-center py-8 text-gray-500">
                                    ...
                                    <p className="text-sm">No transactions have been recorded yet for this recurring income.</p>
                                </div>
                            ) : (
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {transactions.map((tx: any) => (
                                                <tr key={tx.id} className="hover:bg-slate-50">
                                                    ...
                                                    <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-emerald-600 text-right">
