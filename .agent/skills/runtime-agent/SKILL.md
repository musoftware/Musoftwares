---
name: Musoftware Runtime Architecture
description: The local unified runtime agent orchestrating plugins as invisible infrastructure.
---

# Musoftware Runtime Agent

This skill defines the architecture and execution philosophy of the Musoftware Runtime Agent. The Runtime Agent handles the heavy lifting, executing plugins and workflows.

## Activation Conditions
This skill automatically applies when you are:
- Modifying the `musoftware-runtime` NodeJS codebase.
- Implementing IPC (Inter-Process Communication) between Node and Python.
- Managing background orchestration logic.

## 1. Runtime Philosophy: Invisible Infrastructure
The Runtime must be **invisible infrastructure**. 
- Users should barely think about workers, orchestration, IPC, runtimes, or the technical aspects of plugins.
- The experience should feel absolutely **seamless**.
...
The Orchestrator must be intelligent enough to require minimal user configuration.
- **Automatic Recovery**: If a worker crashes, the Orchestrator should catch it and attempt recovery without overwhelming the user with stack traces.
- **Concurrency & Retries**: The runtime should intelligently scale and retry failed API requests based on predefined smart defaults rather than expecting the user to configure technical queue settings.

## 4. Communication Protocols & Security
- **Frontend Isolation**: Frontend NEVER talks directly to workers. All commands flow through the Orchestrator.
- **Localhost-Only**: The Runtime must never bind to public IP interfaces.

## Summary Checklist
- [ ] Is the runtime handling complex logic (retries, crash recovery) internally so the UI doesn't have to?
- [ ] Does the user experience remain seamless, treating the runtime as invisible infrastructure?
- [ ] Are technical errors caught and transformed into actionable, human-readable UI states?
