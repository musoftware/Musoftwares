...
- **Unified Models**: Never use conditional `class_exists()` checks to toggle between module-specific models and main models when dealing with the core `App\Models` namespace. Always unify and use the core models directly from `App\Models`. However, when reading from *other modules*, you MUST use a defensive `class_exists()` + `hasModuleSubscription()` pattern, and you MUST NEVER import another module's model for write operations (use the Event Bus instead).
