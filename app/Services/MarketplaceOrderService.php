...
class MarketplaceOrderService
{
    public function resolveDispute(ServiceOrder $order, string $action): void
    {
        ...
        $escrow = \Modules\Marketplace\Models\MarketplaceEscrow::where('order_id', $order->id)->where('status', 'held')->first();
        ...
        if ($action === 'refund_buyer') {
            ...
            if ($escrow) {
                ...
                if ($buyer) {
                    $transactionId = $buyer->add_balance($escrow->amount, "Refund for cancelled service order #{$order->id} (Dispute)", 'refunded', $escrow->amount_currency);
