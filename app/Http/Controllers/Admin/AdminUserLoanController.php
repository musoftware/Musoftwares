...
class AdminUserLoanController extends Controller
{
    ...
    public function update(Request $request, \App\Models\User $user, \App\Models\UserLoan $loan)
    {
        $request->validate([
            'date' => 'required|date',
            'note' => 'nullable|string'
    ...
    public function destroy(\App\Models\User $user, \App\Models\UserLoan $loan)
    {
        $loan->delete();
        return redirect()->back()->with('success', __('admin.loan_deleted_successfully'));
    }

    public function storeRepayment(Request $request, \App\Models\User $user, \App\Models\UserLoan $loan)
    {
        $request->validate([
            'amount' => 'required|numeric|min:0.01',
            'date' => 'required|date',
