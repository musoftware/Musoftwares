...
class FinanceHelper
{
            ...
            foreach ($charges_per_day as $invoice) {
                ...
                if ($invoice->day == $key) {
                    ...
                    if (! isset($invoices_table[$array_key_client][$invoice->currency])) {
                        $invoices_table[$array_key_client][$invoice->currency] = [
                            'advance' => 0,
                            'due' => 0,
                            'date' => $array_key,
    ...
    public static function timer_cluded($user, $client, $selected_year, $selected_month)
    {
        ...
        $daily_charges = collect(array_fill_keys($days_range, 0))->map(function ($item, $key) use ($charges_per_day, $charges_per_day_sent, $selected_year, $selected_month, $charges_per_day_refund, &$invoices_table) {
            ...
            foreach ($charges_per_day as $invoice) {
                $invoices_table[$array_key][$invoice->currency] = [
                    'advance' => 0,
                    'due' => 0,
                    'work_time' => 0,
                    'date' => $array_key,
    ...
    {
        return static::instance()->format_money($money, CurrenciesExchange::BusinessCurrency());
    }
    ...
    public static function getPaymentMethodsMessage($invoice, $isArabic = true)
    {
        ...
        $unpaidInEGP = CurrenciesExchange::RateToday($unpaid, $invoice->user->currency, 2);
    ...
    public static function calculateOverheadHourlyRate()
    {
        return once(function () {
            ...
            $adjustmentPercent = (int) AdminSettings::GetValue('overhead_cost_default', 150);
     ...
     */
    public function getViewerCurrency(Request $request): Currency
    {
        ...
        $businessCurrencyId = CurrenciesExchange::BusinessCurrency();

        return Currency::find($businessCurrencyId) ?? Currency::where('currency', 'USD')->first();
    }
}
