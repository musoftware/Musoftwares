...
class Project extends Model
{
    ...
    public function hour_rate_client()
    {
        if ($this->client->currency == 2) {
            ...
            $item = \App\Models\Finance\GoldWorldPrice::query()
            ...
            $price_21 = \App\Models\Finance\GoldPrice::query()->where(DB::raw('DATE(price_date)'), $item->price_date)->select(DB::raw('avg(price_21k) as price_21k'))->groupBy(DB::raw('DATE(price_date)'))->first();
    ...
    public function work_time_diff()
    {
        ...
        return $date_start->diffForHumans($date_end, \Carbon\CarbonInterface::DIFF_ABSOLUTE, true);
    }
...
//        if ($finance->count() == 0) {
//            $finance_row = new \App\Models\Marketplace\ProjectFinanceSummary();
//        } else {
...
//    {
//        foreach (\App\Models\Finance\Currency::as_array() as $item) {
...
//    }
    ...
    {
        return $this->currencyRow()->symbol;
    }
