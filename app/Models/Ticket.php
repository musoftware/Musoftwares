...
class Ticket extends Model
{
    ...
    {
        if ($this->ticket_status == 'low') return 'badge-success';
    }

    public function priority_color()
    {
        if ($this->priority == 'low') return 'badge-soft-success';
        if ($this->priority == 'medium') return 'badge-soft-warning';
        if ($this->priority == 'high') return 'badge-soft-danger';
    }
    ...
    {
        if ($this->user_id && Auth::id() != $this->user_id) {
            $this->user->notify(new \App\Notifications\NewTicketMessage($this, $message));
        } else {
            User::admin()->notify(new \App\Notifications\NewTicketMessage($this, $message));
        }
    }
    ...
    {
        // High priority tickets that are not closed
        if ($this->priority === 'high' && $this->ticket_status !== 'closed') {
            return true;
        }

        // Tickets awaiting admin reply
        if ($this->ticket_status === 'user_replied') {
            return true;
        }

        // Tickets open for more than 24 hours without response
