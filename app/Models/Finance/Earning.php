...
class Earning extends Model
{
    ...
    public static function clearing_balance()
    {
        ...
        foreach ($data as $commission) {
            $user_amount = CurrenciesExchange::RateByDate($commission->avg_date, $commission->amount, $commission->currency, \App\Models\Finance\CurrenciesExchange::BusinessCurrency());
            $amount += $user_amount;
        }
        return $amount;
    }
    ...
    public static function total_balance()
    {
        ...
        $amount = 0;
        foreach ($data as $commission) {
            $user_amount = CurrenciesExchange::RateByDate($commission->created_at, $commission->amount, $commission->currency, \App\Models\Finance\CurrenciesExchange::BusinessCurrency());
            $amount += $user_amount;
        }
        return $amount;
    }
