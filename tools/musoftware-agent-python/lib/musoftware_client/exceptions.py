"""
Exceptions for musoftware_client.
All inherit from MusoftwareError for easy catch-all handling.
"""


class MusoftwareError(Exception):
    """Base class for all musoftware_client errors."""


class APIError(MusoftwareError):
...
class NetworkError(MusoftwareError):
    """Cannot reach the server (offline, DNS, timeout)."""


class UpdateError(MusoftwareError):
