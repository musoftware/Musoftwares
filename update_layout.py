...
"""
    ...
    content = content.replace("export default function AdminLayout({ user, children }: AdminLayoutProps) {", "export default function AdminLayout({ user, children }: AdminLayoutProps) {\n    const [isCollapsed, setIsCollapsed] = useState(false);\n    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);\n")
    ...
    new_sidebar = """{/* Desktop Sidebar */}
            <aside className={`hidden flex-col border-r border-gray-200 bg-white md:flex transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-[260px]'}`}>
                <div className="flex h-16 items-center border-b border-gray-200 px-4 flex-shrink-0">
                    {!isCollapsed && <span className="text-xl font-bold text-gray-900 overflow-hidden whitespace-nowrap">Admin Panel</span>}
                </div>
